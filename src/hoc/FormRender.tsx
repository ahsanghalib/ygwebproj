...
function FormRender<T>(props: FormRenderProps<T>) {
    ...
    const handleBlur = (event: any) => {
        ...
        yup.reach(props.validateSchema, name).validate(value).catch(err => {
            setFormErrors({
                ...formErrors,
                [name]: err.message
            })
    ...
    const formValidation = (): Promise<boolean> => {
        return yup.reach(props.validateSchema, '').validate(props.formValues, {abortEarly: false})
            .then(res => {
                return true;
            })
            .catch(err => {
